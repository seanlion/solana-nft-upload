...
//    sellerFeeBasisPoints: number;
  ...
  export const createMixtureV2 = async function (
    ...
    const mixtureAccount = Keypair.generate(); // 부모 NFT 토큰의 account 주소를 seed를 넣자.
  ...
  export async function loadCandyProgram(
    ...
    const program = new anchor.Program(idl as anchor.Idl, CANDY_MACHINE_PROGRAM_ID, provider);
  ...
  export async function loadAuctionHouseProgram(
    ...
    return new anchor.Program(idl as anchor.Idl, AUCTION_HOUSE_PROGRAM_ID, provider);
  }
  ...
  export async function loadTokenEntanglementProgream(
    ...
    return new anchor.Program(idl as anchor.Idl, TOKEN_ENTANGLEMENT_PROGRAM_ID, provider);
  }
